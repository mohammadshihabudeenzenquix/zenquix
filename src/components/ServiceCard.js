    ...
    <div className={`flex-shrink-0 m-6 relative overflow-hidden ${bgColor} rounded-lg max-w-xs shadow-lg`}>
        ...
        <div className="relative text-white px-6 pb-6 mt-6">
            ...
            <div className="flex justify-between">
                <span className="block font-semibold text-xl">{description}</span>
                {price && <span className="block bg-white rounded-full text-black text-xs font-bold px-3 py-2 leading-none flex items-center">{price}</span>}
            </div>
        </div>
    </div>
);
